...
export const LEGAL = {
  ...
  updated: "Last updated 14 September 2026. This is a plain English summary for our pre launch platform, governed by UK GDPR and the Data Protection Act 2018, and is not a substitute for the full terms published at go live.",
  sections: [
    ...
    { t: "Identity and affordability checks", b: "Where we run a liveness identity check, that involves biometric data used to confirm you are who you say you are. We only run this check with your explicit consent, given at the point you apply, and you can withdraw that consent by not proceeding with the check. Where we review Open Banking transaction data for affordability, that review is read only, cannot move money, and a human always makes the final decision on your application. It is never decided by an algorithm alone." },
    { t: "Insurance", b: "Insurance is quoted separately from the rental price by default, through our specialist hire and reward insurance partners. Where an operator holds their own fleet cover and adds it to their listing, the weekly price shown may already include insurance; that cover is self-declared by the operator and not verified by Kharo, so the operator is responsible for keeping it valid. Standard personal motor policies exclude hire and reward, so any policy relied on must properly cover private hire use. Kharo facilitates quotes and payment, and the insurance contract is between you and the insurer." },
    ...
    { t: "Operator verification", b: "Rental companies are checked against the relevant licensing register and Companies House before listing. Operator identity is disclosed to a driver once their application is approved. Where a listing shows insurance as included, that declaration comes from the operator; Kharo is not a party to that policy and does not guarantee its validity." },
    { t: "Who we share data with", b: "We share the minimum data needed with: our email provider, to send you account and welcome messages; our Open Banking partner, to run affordability checks with your consent; our identity verification partner, to run liveness checks with your consent; and our hosting providers, to run the platform. Some of these providers process data outside the UK, in which case we rely on standard contractual safeguards recognised under UK GDPR. We never sell your data, and we never share it with insurers or anyone else for marketing purposes." },
    ...
    { t: "Terms of service, in brief", b: "Using Kharo to browse or register interest is free and creates no obligation on either side. A rental agreement is a contract between you and the rental operator; Kharo is the platform that introduces you and processes payment, not a party to that agreement. You agree to give accurate information, keep your account details to yourself, and not use the platform for anything unlawful. We can suspend an account that breaches these terms or that we reasonably believe is fraudulent. This section is a plain English summary; the full terms of service will be published at go live and will govern in the event of any conflict." },
    { t: "Liability", b: "Kharo is not liable for the acts or omissions of rental operators, insurers, or other third parties on the platform, including where an operator's self-declared insurance turns out to be invalid or lapsed. We are liable for our own negligence and for the accuracy of the checks we say we carry out. Nothing in these terms limits liability for death, personal injury, or fraud." },
  ],
};
...
// SELL and MARKETPLACE content blocks removed along with the car-sale
// marketplace pages - Kharo is a rental marketplace only, no buy/sell side.
