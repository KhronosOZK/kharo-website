    ...
    <main className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
      <div className="flex items-center justify-between gap-3 mb-8 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-heading font-extrabold text-[#0A0A0A]">Compare cars</h1>
            <p className="text-[#888]">{items.length} car{items.length !== 1 ? "s" : ""} side by side</p>
          </div>
        </div>
        {items.length > 0 && (
          <Button onClick={clearCompare} variant="outline" data-testid="compare-clear" className="rounded-full border-[#E0E0E0]">Clear all</Button>
        )}
      </div>

      {items.length === 0 ? (
        <div className="bg-white border border-[#EBEBEB] rounded-3xl p-12 text-center">
          <Columns3 className="w-10 h-10 text-[#CCC] mx-auto" />
          <h2 className="font-heading font-bold text-xl text-[#111] mt-4">Nothing to compare yet</h2>
          <p className="text-[#888] mt-2">Tick the Compare box on any car while you browse, then see them here side by side.</p>
