...
import {
  ...
  CalendarClock, ArrowRight, Headphones, Sparkles, IdCard, LifeBuoy, FileWarning,
} from "lucide-react";
...
// Shown on Insurance/Maintenance/Claims before a driver has an active rental -
// there's nothing real to show yet, so the empty state explains what will
// appear there rather than faking data, matching the "No applications yet"
// pattern already used on this page.
function EmptyTabState({ icon: Icon, title, body, cta, onCta }) {
  return (
    <div className="bg-white rounded-2xl border border-[#0A0A0A]/10 p-10 shadow-sm text-center">
      <Icon className="w-10 h-10 text-[#0B6B4F] mx-auto" strokeWidth={1.5} />
      <p className="text-[15px] text-[#0A0A0A] font-medium mt-3">{title}</p>
      <p className="text-[13px] text-[#888888] mt-1 max-w-sm mx-auto">{body}</p>
      {cta && (
        <Button onClick={onCta} className="rounded-full bg-[#0B6B4F] hover:bg-[#095B43] text-white mt-4 text-[13px] h-9">
          {cta}
        </Button>
      )}
    </div>
...
export default function DriverPortal() {
  ...
  const hasActiveRental = apps.some((a) => a.status === "approved");
      ...
      <Tabs value={tab} onValueChange={setTab} className="mt-7">
        <TabsList className="flex-wrap h-auto bg-white border border-[#0A0A0A]/10 p-1">
          {[
            ...
            <TabsTrigger key={t.v} value={t.v} className="data-[state=active]:bg-[#0A0A0A] data-[state=active]:text-white" data-testid={`driver-tab-${t.v}`}>{t.l}</TabsTrigger>
          ))}
        </TabsList>

        {/* Overview */}
        <TabsContent value="overview" className="mt-6">
          <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6">
            {/* HERO empty state */}
            <motion.div variants={item} className="lg:col-span-8 relative overflow-hidden rounded-3xl bg-[#0A0A0A] text-white min-h-[280px] flex" data-testid="portal-hero">
              ...
              <div className="relative p-7 sm:p-9 flex flex-col justify-center max-w-md [text-shadow:0_2px_16px_rgba(0,0,0,0.5)]">
                ...
                <p className="text-white/70 mt-3 text-[15px] leading-relaxed">Browse vetted cars in London with a rental price shown up front, insurance quoted separately. Apply in minutes with your details saved.</p>
        ...
        <TabsContent value="insurance" className="mt-6">
          {hasActiveRental ? (
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="bg-white rounded-2xl border border-[#0A0A0A]/10 p-6 shadow-sm">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2 text-[#0A0A0A] font-heading font-bold"><ShieldCheck className="w-5 h-5 text-[#0B6B4F]" strokeWidth={1.5} /> Cover</div>
                  <span className="text-[10px] font-bold text-[#0B6B4F] bg-[#EAF5F1] rounded-full px-2 py-0.5">Active</span>
                </div>
                <p className="text-[13px] text-[#888888]">Hire &amp; reward insurance, quoted on your own profile.</p>
                <div className="flex items-baseline gap-1 mt-3">
                  <span className="font-heading font-extrabold text-2xl text-[#0A0A0A]">£38</span>
                  <span className="text-[#AAA] text-[12px]">/ week, indicative</span>
                </div>
                <Button variant="outline" className="rounded-full border-[#0A0A0A]/20 text-[13px] h-9 mt-4">View certificate</Button>
              </div>
              <div className="bg-white rounded-2xl border border-[#0A0A0A]/10 p-6 shadow-sm">
                <div className="text-[12px] font-semibold text-[#888] uppercase tracking-wide">Cover &amp; compliance</div>
                <div className="mt-3 space-y-2.5 text-[13.5px]">
                  ...
                  <div className="flex items-center justify-between"><span className="text-[#666666]">Renewal</span><span className="text-[#888888]">Weekly, automatic</span></div>
                </div>
              </div>
            </div>
          ) : (
            <EmptyTabState
              icon={ShieldCheck}
              title="No active cover yet"
              body="Once you're approved for a car, your insurance quote, certificate and renewal date live here. Your rental price is shown separately - insurance is quoted on your own profile."
              cta="Browse cars"
        ...
        <TabsContent value="maintenance" className="mt-6">
          <div className="grid sm:grid-cols-4 gap-3 mb-4">
            {[
              ...
              <button key={a.t} onClick={() => navigate("/help")} className="rounded-2xl bg-white hover:bg-[#F5F5F5] border border-[#0A0A0A]/10 p-4 text-left transition-colors hover:-translate-y-[2px] shadow-sm">
          ...
          <EmptyTabState
            ...
            body="Once you're in a car, report a breakdown, warning light or service need here and track it through to resolved - no chasing a phone line."
          />
          {!hasActiveRental && <p className="text-[12px] text-[#999999] text-center mt-3">These become active the moment your rental starts.</p>}
        </TabsContent>

        {/* Claims */}
        <TabsContent value="claims" className="mt-6">
          <div className="flex justify-end mb-4">
            <Button onClick={() => navigate("/help")} className="rounded-full bg-[#0B6B4F] hover:bg-[#095B43] text-white text-[13px] h-9"><FileWarning className="w-4 h-4 mr-2" /> Report a claim</Button>
          </div>
          <EmptyTabState
            icon={LifeBuoy}
            title="No claims yet"
            body="If you're ever in an accident, or your car is affected by theft or damage, start it here. We call you within 30 minutes and track it through to settlement."
          />
        </TabsContent>
        ...
        <TabsContent value="documents" className="mt-6">
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div className="bg-white rounded-2xl border border-[#0A0A0A]/10 p-6 shadow-sm">
              <div className="text-[12px] font-semibold text-[#888] uppercase tracking-wide mb-3">Once you're in a car</div>
              <div className="space-y-2.5 text-[13.5px] text-[#666666]">
                <div className="flex items-center gap-2"><FileText className="w-3.5 h-3.5 text-[#AAA]" /> Rental agreement</div>
                <div className="flex items-center gap-2"><FileText className="w-3.5 h-3.5 text-[#AAA]" /> Insurance certificate</div>
                <div className="flex items-center gap-2"><FileText className="w-3.5 h-3.5 text-[#AAA]" /> Handover condition photos</div>
              </div>
              <p className="text-[12px] text-[#999999] mt-4">These appear here automatically once an operator approves your application.</p>
            </div>
          </div>
        </TabsContent>
      </Tabs>
