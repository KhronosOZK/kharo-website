...
function LiveMap() {
  return (
    <div className="grid lg:grid-cols-[1.6fr_1fr] gap-4">
      <div className="relative rounded-2xl overflow-hidden bg-[#0A0A0A] min-h-[360px]" data-testid="fleet-map">
        ...
        <div className="absolute top-4 left-4 flex items-center gap-2 rounded-full bg-[#0A0A0A]/70 px-3 py-1.5 text-white text-[12px] font-medium border border-white/10">
        ...
        <div className="absolute top-4 right-4 flex items-center gap-1.5 rounded-full bg-white/95 px-3 py-1.5 text-[11px] font-semibold text-[#0A0A0A] shadow-sm" data-testid="tracking-launch-badge">
        ...
        {vehicles.map((v) => (
          <div key={v.plate} className="absolute -translate-x-1/2 -translate-y-1/2 group" style={{ left: `${v.x}%`, top: `${v.y}%` }}>
            ...
            <div className="relative w-7 h-7 rounded-full flex items-center justify-center shadow-lg" style={{ background: pinColor(v.status) }}>
              <Navigation className="w-3.5 h-3.5 text-[#0A0A0A]" strokeWidth={2} />
            </div>
            <div className="absolute left-1/2 -translate-x-1/2 mt-1.5 whitespace-nowrap rounded-lg bg-[#0A0A0A] text-white text-[11px] px-2 py-1 opacity-0 group-hover:opacity-100 transition-opacity">{v.name} · {v.speed}</div>
          </div>
        ))}
        <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between text-white/70 text-[12px]">
          ...
          <span className="flex items-center gap-4">
            ...
            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-[#555555]" /> Idle</span>
            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-[#999999]" /> At depot</span>
          </span>
        </div>
      </div>
      <div className="bg-white rounded-2xl border border-[#0A0A0A]/10 p-5 shadow-sm">
        <div className="flex items-center gap-2 text-[#0A0A0A] font-heading font-bold mb-3"><Radio className="w-5 h-5 text-[#0B6B4F]" strokeWidth={1.5} /> Live vehicles</div>
        <div className="space-y-2.5">
          {vehicles.map((v) => (
            <div key={v.plate} className="flex items-center justify-between border border-[#0A0A0A]/8 rounded-xl p-3">
              <div className="flex items-center gap-2.5">
                <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: pinColor(v.status) }} />
                <div><div className="text-[13px] font-medium text-[#0A0A0A]">{v.name}</div><div className="text-[11.5px] text-[#888888] flex items-center gap-1"><MapPin className="w-3 h-3" /> {v.area}</div></div>
              </div>
              <div className="text-right"><div className="text-[12px] font-semibold text-[#0A0A0A]">{v.status}</div><div className="text-[11px] text-[#888888]">{v.speed}</div></div>
            </div>
          ))}
        </div>
      </div>
      ...
      <Tabs value={tab} onValueChange={setTab} className="mt-6">
        ...
        <TabsContent value="overview" className="mt-6">
          <motion.div initial="hidden" animate="show" variants={{ show: { transition: { staggerChildren: 0.05 } } }} className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {kpis.map((k) => (
              <motion.div key={k.l} variants={{ hidden: { opacity: 0, y: 16 }, show: { opacity: 1, y: 0 } }} className="bg-white border border-[#0A0A0A]/10 rounded-2xl p-5 shadow-sm">
                <div className="flex items-center justify-between"><k.i className="w-5 h-5 text-[#0B6B4F]" strokeWidth={1.5} />{k.up && <TrendingUp className="w-4 h-4 text-[#0B6B4F]" />}</div>
                <div className="text-2xl font-heading font-extrabold text-[#0A0A0A] mt-3">{k.v}</div>
                <div className="text-xs text-[#666666]">{k.l}</div>
                <div className="text-xs text-[#0B6B4F] mt-1">{k.d}</div>
              </motion.div>
            ))}
          </motion.div>
          ...
          <div className="mt-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-heading font-bold text-[#0A0A0A] text-lg flex items-center gap-2"><MapPin className="w-5 h-5 text-[#0B6B4F]" strokeWidth={1.5} /> Live vehicle tracking</h3>
          ...
          <div className="grid lg:grid-cols-3 gap-4 mt-4">
            <div className="lg:col-span-2 bg-white border border-[#0A0A0A]/10 rounded-2xl p-5 shadow-sm">
              <h3 className="font-heading font-bold text-[#0A0A0A] mb-4">Net revenue by vehicle (this month)</h3>
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={revChart}>
                  <XAxis dataKey="name" tickLine={false} axisLine={false} fontSize={12} stroke="#666666" />
                  <Tooltip cursor={{ fill: "#F0F0F0" }} formatter={(x) => [`£${x}`, "Net"]} />
        ...
        <TabsContent value="tracking" className="mt-6">
          <div className="mb-4"><h3 className="font-heading font-bold text-[#0A0A0A] text-lg">Where your fleet is right now</h3><p className="text-[13px] text-[#666666] mt-1">Every rented car reports its location so you always know where your assets are. Tap a pin for detail.</p></div>
          <LiveMap />
        </TabsContent>
        ...
        <TabsContent value="financials" className="mt-6">
          ...
          <div className="bg-white border border-[#0A0A0A]/10 rounded-2xl p-5 shadow-sm">
            <div className="flex items-center justify-between mb-3"><h3 className="font-heading font-bold text-[#0A0A0A]">Revenue by vehicle, this month</h3><Button size="sm" variant="outline" className="rounded-full border-[#0A0A0A]/20">Export CSV</Button></div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[420px]">
                <thead><tr className="text-left text-[#888888] text-xs border-b border-[#0A0A0A]/8"><th className="py-2">Vehicle</th><th>Weeks</th><th>Gross</th><th className="text-right">Net (after 10%)</th></tr></thead>
                <tbody>{revenue.map((r) => (<tr key={r[0]} className="border-b border-[#0A0A0A]/8 last:border-0"><td className="py-2.5 text-[#0A0A0A] font-medium">{r[0]}</td><td>{r[1]}</td><td>{r[2]}</td><td className="text-right text-[#0B6B4F] font-semibold">{r[3]}</td></tr>))}</tbody>
              </table>
            </div>
          </div>
        </TabsContent>
...
const Stat = ({ l, v, hint }) => (<div className="bg-white border border-[#0A0A0A]/10 rounded-2xl p-5 shadow-sm"><div className="text-xs text-[#888888]">{l}</div><div className="text-xl font-heading font-extrabold text-[#0A0A0A] mt-1">{v}</div><div className="text-xs text-[#999999] mt-0.5">{hint}</div></div>);
