      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 py-12 lg:py-20 grid lg:grid-cols-2 gap-12 lg:gap-16 items-start">
        ...
        <div>
          <motion.div initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <h1 className="mt-3 font-heading font-extrabold tracking-tight text-[30px] sm:text-5xl lg:text-6xl leading-[1.06] text-[#111] text-balance">
              {SELL.hero.heading}
            </h1>
            <p className="mt-4 text-[16px] text-[#666] max-w-md leading-relaxed">{SELL.hero.sub}</p>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.5 }}
            ...
            <div className="grid grid-cols-3 gap-4 mt-4">
              <div>
                ...
                <div className="text-[12.5px] text-[#666] mt-1.5">looking to buy</div>
              </div>
              <div>
                <AnimatedNumber value={demand?.sellers ?? 0} className="text-[34px] font-heading font-extrabold text-[#111] leading-none block" />
                <div className="text-[12.5px] text-[#666] mt-1.5">looking to sell</div>
              </div>
              <div>
                <AnimatedNumber value={demand?.listings ?? 0} className="text-[34px] font-heading font-extrabold text-[#111] leading-none block" />
                <div className="text-[12.5px] text-[#666] mt-1.5">vehicles listed</div>
              </div>
            </div>
            <p className="text-[12.5px] text-[#888] mt-4 leading-relaxed">
            ...
            {SELL.points.map((p) => {
              ...
              return (
                <div key={p.t} className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="font-heading font-bold text-[#111] text-[15px]">{p.t}</div>
                    <p className="text-[14px] text-[#666] mt-0.5 leading-relaxed">{p.d}</p>
                  </div>
                </div>
              );
            })}
        ...
        <div className="w-full lg:justify-self-end lg:max-w-md bg-white rounded-[28px] ring-1 ring-gray-200 p-6 sm:p-8 shadow-xl">
          ...
          <div className="mt-5 mb-6 rounded-2xl bg-[#EAF5F1] border border-[#0B6B4F]/15 p-3.5">
            <p className="text-[13px] text-[#111] leading-relaxed">{SELL.form.note}</p>
          </div>
