...
export default function MarketplaceDetail() {
  ...
  if (notFound) return (
    <main className="max-w-3xl mx-auto px-4 py-24 text-center">
      <h1 className="text-3xl font-heading font-extrabold text-[#111]">That vehicle is no longer listed.</h1>
      <p className="text-[#666] mt-3">It may have sold. Browse what is available now.</p>
  ...
  return (
    <main className="bg-[#FAFAFA] pb-28 lg:pb-16">
      {/* Breadcrumb */}
      <nav aria-label="Breadcrumb" className="max-w-7xl mx-auto px-4 sm:px-6 pt-5">
        <ol className="flex items-center gap-1.5 text-[13px] text-[#888] flex-wrap">
          ...
          <Chev className="w-3.5 h-3.5" aria-hidden />
          <li className="text-[#111] font-medium truncate">{title}</li>
        </ol>
      </nav>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 mt-4 flex items-center justify-between gap-3">
        <button onClick={() => navigate("/marketplace")} data-testid="md-back"
          className="inline-flex items-center gap-1.5 text-sm text-[#666] hover:text-[#0B6B4F]">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => toggleSavedSale(v.id)} data-testid="md-save" aria-pressed={isSaved}
            className="inline-flex items-center gap-1.5 text-sm text-[#666] hover:text-[#0B6B4F]">
            <Heart className={`w-4 h-4 ${isSaved ? "fill-[#111] text-[#111]" : ""}`} /> {isSaved ? "Saved" : "Save"}
          </button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 mt-4 grid lg:grid-cols-[1.55fr,1fr] gap-8 lg:gap-12">
        {/* LEFT */}
        <div>
          <div className="relative rounded-[26px] overflow-hidden bg-[#EBEBEB] aspect-[16/10] group">
              ...
              {licensing.map((row) => (
                <div key={row.l} className="bg-white rounded-2xl ring-1 ring-[#E8E8E8]/70 p-4 flex gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="font-heading font-bold text-[#111] text-[15px] mt-0.5">{row.v}</div>
                    {row.note && <div className="text-[12.5px] text-[#666] mt-0.5">{row.note}</div>}
                  </div>
                </div>
              ))}
            </div>
            <div className="mt-3 rounded-2xl bg-[#EAF5F1] border border-[#0B6B4F]/15 p-4 flex gap-3">
              <AlertCircle className="w-5 h-5 text-[#0B6B4F] shrink-0 mt-0.5" strokeWidth={1.7} />
              <p className="text-[13.5px] text-[#111] leading-relaxed">{MARKETPLACE.pcoNote.body}</p>
            </div>
          </section>
          ...
          {/* DESCRIPTION */}
          <section className="mt-9">
            <h2 className="text-xl font-heading font-bold text-[#111]">{MARKETPLACE.detail.descriptionHeading}</h2>
            <p className="text-[#666] mt-3 text-[16px] leading-relaxed">{v.description}</p>
            {v.features?.length > 0 && (
              <>
          ...
          {/* BUYER CHECKLIST */}
          <section className="mt-9">
            <h2 className="text-xl font-heading font-bold text-[#111]">{MARKETPLACE.detail.checklistHeading}</h2>
        ...
        <aside className="lg:sticky lg:top-24 lg:self-start" ref={enquiryRef}>
          <div className="bg-white rounded-[26px] ring-1 ring-[#E8E8E8] p-6 sm:p-7 shadow-xl">
            ...
            {ctx && (
              <div className={`mt-3 rounded-2xl p-3 flex gap-2.5 items-start ${wellPriced ? "bg-[#EAF5F1]" : "bg-[#FAFAFA]"}`} data-testid="md-price-context">
                <TrendingDown className={`w-4 h-4 shrink-0 mt-0.5 ${wellPriced ? "text-[#0B6B4F]" : "text-[#888]"}`} strokeWidth={1.8} />
                <p className="text-[12.5px] text-[#111] leading-relaxed">
                  {wellPriced
      ...
      {v.similar?.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 mt-16" data-testid="md-similar">
          <h2 className="text-2xl sm:text-3xl font-heading font-bold text-[#111]">Similar vehicles for sale</h2>
          <p className="text-[#666] mt-1.5">Comparable cars at a similar price, so you can weigh them side by side.</p>
      ...
      <div className="lg:hidden fixed bottom-0 inset-x-0 z-40 bg-white/95 backdrop-blur-xl border-t border-[#E8E8E8] px-4 py-3 flex items-center justify-between gap-3"
        ...
        <div className="min-w-0">
          ...
          <div className="text-[11.5px] text-[#888] mt-1 truncate">{badge.text} · {formatMileage(v.mileage)}</div>
        </div>
