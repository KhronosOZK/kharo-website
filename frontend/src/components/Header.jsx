      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-[68px] flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-7 lg:gap-9">
          {NAV.primary.map((l) => (
            <Link key={l.to} to={l.to} data-testid={`nav-${l.label.toLowerCase().replace(/\s+/g, "-")}`}
              className="text-[14px] font-medium text-gray-600 hover:text-[#0B6B4F] transition-colors whitespace-nowrap">
              {l.label}
            </Link>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <button onClick={() => navigate("/saved")} aria-label={`Saved vehicles, ${savedTotal} items`}
            className="hidden sm:inline-flex items-center gap-1.5 text-[14px] text-gray-600 hover:text-[#0B6B4F] transition-colors" data-testid="saved-count">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button data-testid="account-menu-btn"
                className="hidden md:inline-flex items-center gap-2.5 rounded-full border border-gray-300 bg-white pl-3.5 pr-2 py-1.5 hover:shadow-md transition-shadow">
            ...
            <DropdownMenuContent align="end" className="w-72 bg-white shadow-xl border-gray-200 p-1.5">
              ...
              <DropdownMenuItem onClick={() => navigate("/help")} className="cursor-pointer py-2.5 rounded-lg text-gray-600">Get help</DropdownMenuItem>
              <DropdownMenuItem onClick={() => navigate("/legal")} className="cursor-pointer py-2.5 rounded-lg text-gray-600">Legal and privacy</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
            ...
            <SheetContent side="right" className="w-80 bg-white overflow-y-auto">
              ...
              <div className="flex flex-col gap-1">
                <button onClick={() => { setOpen(false); navigate("/saved"); }} data-testid="mobile-saved-link"
                  className="py-3 px-2 text-base font-medium text-[#0A0A0A] hover:bg-gray-50 rounded-lg flex items-center justify-between">
                  <span className="flex items-center gap-2.5"><Heart className="w-[18px] h-[18px] text-gray-500" /> Saved vehicles</span>
                ...
                {NAV.primary.map((l) => (
                  <Link key={l.to} to={l.to} onClick={() => setOpen(false)}
                    className="py-3 px-2 text-base font-medium text-[#0A0A0A] hover:bg-gray-50 rounded-lg">{l.label}</Link>
                ))}
                ...
                <div className="border-t border-gray-200 my-3" />
                <Link to="/why-caro" onClick={() => setOpen(false)} className="py-3 px-2 text-gray-600">Why choose {BRAND.name}</Link>
                <Link to="/help" onClick={() => setOpen(false)} className="py-3 px-2 text-gray-600">Get help</Link>
                <Link to="/legal" onClick={() => setOpen(false)} className="py-3 px-2 text-gray-600">Legal and privacy</Link>
              </div>
            </SheetContent>
          </Sheet>
        </div>
